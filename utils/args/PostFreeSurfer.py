...
def build(context):
    ...
    params['surfatlasdir'] = op.join(environ['HCPPIPEDIR_Templates'],
                                     'standard_mesh_atlases')
    # (Need to copy these in)$GrayordinatesSpaceDIR
    params['grayordinatesdir'] = op.join(environ['HCPPIPEDIR_Templates'],
                                         config['GrayordinatesTemplate'])
    ...
    params['subcortgraylabels'] = op.join(
                                    environ['HCPPIPEDIR_Config'],
                                    'FreeSurferSubcorticalLabelTableLut.txt')
    params['freesurferlabels'] = op.join(environ['HCPPIPEDIR_Config'],
                                         'FreeSurferAllLut.txt')
    params['refmyelinmaps'] = op.join(environ['HCPPIPEDIR_Templates'],
                                'standard_mesh_atlases',
