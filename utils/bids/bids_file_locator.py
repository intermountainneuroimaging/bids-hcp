...
from pybids.bids.layout import BIDSLayout

from bids_hcp_struct.bids import validate, run_level

from .custom_logger import get_custom_logger

log = logging.getLogger(__name__)

def gather_bids_files(gtk_context):
    """
    ...
    """
    gtk_context = GearToolkitContext() # Move to gear preliminaries?
    hierarchy = run_level.get_run_level_and_hierarchy(gtk_context.client, gtk_context.destination['id'])

    bids_dir = op.join(gtk_context.work_dir, 'bids')
    grab_BIDS_data(gtk_context)

    layout = BIDSLayout(bids_dir, derivatives=False, absolute_paths=True)
...
def find_t1ws():
    subject_label = hierarchy['run_label']
...
def find_struct_fieldmaps():
    ...
    if fieldmap_set[0]["suffix"] == "phasediff":
        ...
        merged_file = "%s/tmp/%s/magfile.nii.gz" % (bids_dir, subject_label)
        sp.run(["mkdir", "-p", op.join(bids_dir, subject_label),
...
def grab_BIDS_data(gtk_context):
    """
    ...
    try:
        gtk_context.download_session_bids(target_dir=gtk_context.work_dir / "bids")
    except Exception as e:
        log.error(
            f'Could not download BIDS data from {gtk_context.work_dir / "bids"}'
        )
        log.exception(e)
    ...
    if config["gear-run-bids-validation"] and config["gear-abort-on-bids-error"]:
        ...
        #validate.validate_bids(gtk_context)
